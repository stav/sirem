...
const ENROLLMENT_STATUSES_ARRAY = [
  'pending',
  'active',
  'cancelled',
  'terminated',
...
export function parseLegacyPlanType(legacyType: string): PlanTypeStructure | null {
  ...
  if (upper.includes('SUPPLEMENT')) return { type_network: 'HMO', type_extension: null, type_snp: null, type_program: 'Supplement' }
  if (upper.includes('ANCILLARY')) return { type_network: 'HMO', type_extension: null, type_snp: null, type_program: 'Ancillary' }
...
export function mapCarrier(value: string | null | undefined): Carrier | null {
  ...
  const found = CARRIERS.find(carrier => 
    carrier.toLowerCase() === normalized.toLowerCase()
  )
  return found || 'Other'
}