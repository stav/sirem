 ...
 */
export function calculateCmsId(
  plan: Pick<Plan, 'cms_contract_number' | 'cms_plan_number' | 'cms_geo_segment'>
): string | null {
 ...
 */
export function buildPlanTypeString(
  plan: Pick<Plan, 'type_network' | 'type_extension' | 'type_snp' | 'type_program'>
): string {
  const parts = []
...
export function formatPlanDisplayName(
  plan: Pick<Plan, 'carrier' | 'name' | 'plan_year' | 'cms_contract_number' | 'cms_plan_number' | 'cms_geo_segment'>
): string {
