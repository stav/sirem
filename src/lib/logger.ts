...
export const useLogger = create<LoggerStore>((set, get) => ({
  ...
  addMessage: (message: string, type: 'success' | 'error' | 'info' | 'warning' = 'info', action?: string, details?: Record<string, unknown>) => {
...
export const logger = {
  contactUpdated: (contactName: string) => {
    useLogger.getState().addMessage(
      `Contact record updated for ${contactName}`,
      'success',
      'contact_update',
      { contactName }
    )
  ...
  contactCreated: (contactName: string) => {
    useLogger.getState().addMessage(
      `New contact created: ${contactName}`,
      'success',
      'contact_create',
      { contactName }
    )
  ...
  reminderCreated: (reminderTitle: string) => {
    useLogger.getState().addMessage(
      `Reminder created: ${reminderTitle}`,
      'success',
      'reminder_create',
      { reminderTitle }
    )
  ...
  reminderCompleted: (reminderTitle: string) => {
    useLogger.getState().addMessage(
      `Reminder completed: ${reminderTitle}`,
      'success',
      'reminder_complete',
      { reminderTitle }
    )
